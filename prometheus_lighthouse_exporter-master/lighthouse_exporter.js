...
http.createServer(async (req, res) => {
    ...
    if(q.pathname == '/probe'){
        ...
        try{
            // Added '--user-agent' to args for testing SP view. 
            // ref: https://peter.sh/experiments/chromium-command-line-switches/
            const userAgent = 'Mozilla/5.0 (iPhone; CPU iPhone OS 12_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/12.0 Mobile/15E148 Safari/604.';
            const browser = await puppeteer.launch({args: ['--no-sandbox', '--disable-setuid-sandbox', `--user-agent=${userAgent}`]});

            data.push('# HELP lighthouse_exporter_info Exporter Info');
            data.push('# TYPE lighthouse_exporter_info gauge');
            data.push(`lighthouse_exporter_info{version="0.2.6",user_agent="${await browser.userAgent()}", chrome_version="${await browser.version()}",node_version="${process.version}"} 1`);
